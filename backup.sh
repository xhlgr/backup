...
#Add delete local zip file function.
#Delete FTP function.
#
...
LOGFILE="/var/www/fastuser/data/www/zbackup/log/backup.log"
...
clean_up_files() {
    #cd ${LOCALDIR} || exit
    #LS=($(ls *.zip))
    LS=($(rclone ls ${RCLONE_NAME}:${RCLONE_FOLDER}))
    for f in ${LS[@]}; do
        get_file_date ${f}
        if [ $? -eq 0 ]; then
...
delete_local_files() {
    if ${DELETE_LOCAL_FILE_FLG} && [ -n ${OUT_FILE} ]; then
        rm -f ${OUT_FILE}
        log "Local file: ${OUT_FILE} has been deleted"
    fi
}
